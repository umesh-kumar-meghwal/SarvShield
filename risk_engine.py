...
def calculate_risk(
    ...
    **kwargs
    ...
    verdict = (
        "VERY HIGH RISK" if final_score >= 80 else
        "HIGH RISK" if final_score >= 60 else
        "SUSPICIOUS" if final_score >= 30 else
        "LOW RISK"
    )
...
def what_if_analysis(message_score=0, phone_score=0, link_score=0, screenshot_score=0) -> dict:
    ...
    return {
        ...
        "phone_only": calculate_risk(0, p, 0, 0)["final_score"] if p > 0 else 0,
